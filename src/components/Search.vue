...
const filteredMems = computed(()=>{
  return props.members?.filter(member => member.languages.find(lang => lang.name === language.value)).sort((a, b) => {
      ...
      return 0;
    }).map(mem =>( {id: mem.id, name: mem.name || mem.login, projectCount: mem.languages.find(lang => lang.name === language.value)?.projectCount || 0} ))  || []
})
</script>
